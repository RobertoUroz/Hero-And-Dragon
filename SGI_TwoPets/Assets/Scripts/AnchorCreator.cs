...
public class AnchorCreator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (m_RaycastManager.Raycast(touch.position, s_Hits, TrackableType.Depth))
        {
            ...
            if (isCat)
            {
                if (m_AnchorPoints.Count >= 2)
                {
                    m_AnchorPoints.RemoveAt(0);
                }
                Instantiate(m_CatPrefab, anchor.transform);
                isCat = false;
            }
            else
            {
                ...
                Instantiate(m_DogPrefab, anchor.transform);
                isCat = true;
            }
